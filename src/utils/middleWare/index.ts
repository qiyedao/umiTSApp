...
declare module 'umi-request' {
  ...
  export interface RequestOptionsInit {
     ...
     * 下载进度事件的回调函数
     */
    onDownloadProgress?: ProgressHandler | null;
  }
}
 ...
 * import progressMiddleware from '.';
 *
 * // 注册内核中间件
 ...
 *    },
 *    //下载进度事件的回调函数
 *    onDownloadProgress:function( ev: ProgressEvent){
 *      console.log(ev)
 *    },
 * });
 * ```
...
const progressMiddleware: OnionMiddleware = async function (ctx, next) {
  ...
  if (!(options.onUploadProgress || options.onDownloadProgress)) {
    await next();
    return;
  }
